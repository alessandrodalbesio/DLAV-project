...
from motionnet.models.qcnet.layers import AttentionLayer
from motionnet.models.qcnet.layers import FourierEmbedding
from motionnet.models.qcnet.layers import MLPLayer
from motionnet.models.qcnet.utils import angle_between_2d_vectors
from motionnet.models.qcnet.utils import bipartite_dense_to_sparse
...
class QCNetDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # mode embedding: needed for mode queries ( categorical data )
    ...
    def forward(self,
        ...
        # ENCODING 
        # Update mode queries with multiple contexts: target agent (1), history, map elements(2) and neighbors elements(3)
        ...
        mask_src = data['agent']['valid_mask'][:, :self.num_historical_steps].contiguous()           # src stands for source
        mask_src[:, :self.num_historical_steps - self.num_t2m_steps] = False
        mask_dst = data['agent']['predict_mask'].any(dim=-1, keepdim=True).repeat(1, self.num_modes) # dst stands for destination

        # Targer agent embedding (1)
        pos_t = data['agent']['position'][:, :self.num_historical_steps, :self.input_dim].reshape(-1, self.input_dim) #--prepare for target agent to mode query--
        ...
        rel_pos_t2m = pos_t[edge_index_t2m[0]] - pos_m[edge_index_t2m[1]]                                             # relative position
        rel_head_t2m = wrap_angle(head_t[edge_index_t2m[0]] - head_m[edge_index_t2m[1]])                              # relative heading
        r_t2m = torch.stack(                                                                                          
        ...
        r_t2m = self.r_t2m_emb(continuous_inputs=r_t2m, categorical_embs=None)                                       # --continuous embedding--
        ...
        # Map elements embedding (2)   
        pos_pl = data['map_polygon']['position'][:, :self.input_dim]                                                #--prepare for map element to mode query--
        ...
        r_pl2m = torch.stack(                                                                                       # data prepared
        ...
        r_pl2m = self.r_pl2m_emb(continuous_inputs=r_pl2m, categorical_embs=None)                                  #--continuous embedding--
        ...
        r_a2m = torch.stack(                                                                                     # data prepared
            [torch.norm(rel_pos_a2m[:, :2], p=2, dim=-1),
             ...
             rel_head_a2m], dim=-1)
        r_a2m = self.r_a2m_emb(continuous_inputs=r_a2m, categorical_embs=None)                                  #--continuous embedding--
        ...
        # DECODING TRAJECTORY PROPOSAL: anchor free
        # initialize the proposal distribution: locs_pos is the location of the agent, scales_pos is the scale of the agent, locs_head is the agent's direction, concs_head is the concentration (see """variance""") of direction
        ...
        # compute the cumulative sum of the proposal distribution to get the final distribution
        ...
        scale_propose_pos = torch.cumsum(
            ...
            1.0,
            dim=-2) + 0.1
        # compute the cumulative sum of the proposal distribution for the head
        if self.output_head:
        ...
        # m represents the possible future trajectory of the target agent, they are the anchor points for the next step


        # DECODING TRAJECTORY REFINEMENT: anchor based
        # apply cross attention layers
        for i in range(self.num_layers):
        ...
        # MLP LAYER: get the probability of each mode
